    ...
    public class EService : IEService
    {
        ...
        public string VoteClose()
        {
            ...
            if (fVote != null)
            {
                fVote.t.Enabled = false;
                id = fVote._xitiId;
                answer = fVote.VoteOption;
                result = fVote.getResult();
                createtime = fVote.tm_create.ToString("yyyyMMddHHmmss");
                timeuse = Util.getTimeDiff_Second(fVote.tm_create);
                //通过xitiResult获取timeAverage等各种时长
                XitiResult VoteResult = new XitiResult(id, answer, result, "", "", "", createtime, timeuse);
                Common.uploadVoteResult(VoteResult);
                fVote.Dispose();
                fVote = null;
            }
            return "success";
        ...
        public string QDStart()
        {
            ...
            Global.g_QDStu.Clear();
            Global.g_QDtm = fQiangDa.tm_create;
            return "success";
        }
        ...
        {
            string result = "", id = "", answer = "", createtime = "",attendStu="";
            int timeuse = 0;
            if (fQiangDa != null)
            {
                id = fQiangDa._xitiId;
                createtime = fQiangDa.tm_create.ToString("yyyyMMddHHmmss");
                timeuse = Util.getTimeDiff_Second(fQiangDa.tm_create);

                Global.g_QDResult += ":" + createtime + ":" + Global.g_QDRtm;
                result = Global.g_QDResult;
                Global.g_QDStu = Global.g_QDStu.Distinct().ToList();
                foreach (string s in Global.g_QDStu)
                {
                    attendStu += s + ":";
                }
                ...
                Common.uploadQDResult(id,createtime,timeuse,result,attendStu);
                Httpd.cancelHandon();
                fQiangDa.Dispose();
                fQiangDa = null;
                Global.g_QDStu.Clear();
